...
func (c *ArticleController) TurnToArticleList() {
	...
	logs.Info("下拉框选中的论文别名为：", typeName)
	...
	if typeName == "" { // 默认查询所有
		...
		if typeName == "" {
			...
			// 没有类型名就查询所有
			total, _ = qs.Count()
			logs.Info("查询论文的总记录数：", total)
			_, err = qs.Limit(pageSize, pageSize*(pageIndex-1)).All(&articles)
			if err != nil {
				logs.Info("查询所有论文信息失败，err=", err)
				return
			}
		} else {
			logs.Info("当前默认论文类型为：", typeName)
			qs = qs.Filter("Atype__Typename", typeName)
			total, _ = qs.Count()
			_, err = qs.Limit(pageSize, pageSize*(pageIndex-1)).All(&articles)
			logs.Info("查询的总记录数：", total)
			if err != nil {
				logs.Info("查询所有论文信息失败，", err)
				return
			}
		}
		// return
	} else { // 根据类型字段来查询
		logs.Info("选中的论文类型为：", typeName)
		qs = qs.Filter("Atype__Typename", typeName)
		total, _ = qs.Count()
		_, err = qs.Limit(pageSize, pageSize*(pageIndex-1)).All(&articles)
		logs.Info("查询的总记录数：", total)
	...
	// Limit()函数，参数1：一个页面显示的论文条数，参数2：偏移量，下标从0开始
...
func (c *ArticleController) TurnToArticleDetail() {
	...
	/*arti.Id = id
	  err = o.Read(&arti)*/
	if err != nil {
		logs.Info("查询论文失败，err=", err)
		return
	}
	// 3、传递数据给视图
	c.Data["article"] = arti
	logs.Info("当前论文内容为：", arti, "论文类型为：", arti.Atype.Typename)
	c.Layout = "layout.html"
	c.LayoutSections = make(map[string]string)
